...
```
TARGETS="Firewalla:192.168.1.1:1,Deco:192.168.1.2:1,Cloudflare:1.1.1.1,Cloudflare DNS:1.1.1.1:dns"
...
Each target has a probe kind: `icmp` (default; echo RTT; accepted alias `ping`) or `dns` — a real recursive A query for a well-known name over UDP :53, timing until the answer arrives. DNS probes share the ping pipeline: the query RTT is a sample, a timeout or non-NOERROR rcode is a lost sample, so loss stats and outage detection apply unchanged. Uniqueness is (host, probe), so the same resolver can be monitored both ways. The env form is `Name:host[:tier][:probe]` — after host, a bare 1-3 is the tier (default 3), a name is the probe, in either order.
